...
export function ProductDetail() {
  ...
  const validateFormFields = useMemo(() => {
    return () => {
      ...
      // Conditional validators based on product configuration
      const conditionalValidators = {};
      
      if (product?.lightBases?.length > 0) {
        conditionalValidators.lightBase = {
          isValid: (value) => !!value,
          message: 'Please select a light base'
        };
      }

      if (product?.giftStand?.length > 0) {
        conditionalValidators.giftStand = {
          isValid: (value) => !!value,
          message: 'Please select a stand'
        };
      }

      const validators = { ...commonValidators, ...conditionalValidators };
            ...
            <div className="col-12 col-sm-12 col-md-7 col-lg-6">          
              ...
              <Form className="product-options" onSubmit={handleSubmit}> 
                ...
                {product.giftStand && product.giftStand.length > 0 && (
                  <Form.Group className="product-option pt-2 mt-2">
                    <Form.Label ref={standRef}>Stand <span className="text-danger">*</span></Form.Label>
