...
// Version: 2.1.0 - 2025-11-06 - LIGHTBASE OPTIONS FROM PRODUCT ONLY
// ✅ Fixed: Lightbases now extracted from product's OWN options, not catalog
// ✅ Fixed: Products without lightbase options won't show any lightbase options
// ✅ Added: extractProductLightbases() method to read from product options array
// Previous: All non-keychain products got catalog lightbases (incorrect)
...
class CockPit3DFetcher {
    ...
    public function __construct() {
        ...
        if (!file_exists($this->cacheDir)) {
            if (mkdir($this->cacheDir, 0755, true)) {
                ...
                console_log("âŒ Failed to create cache directory");
            }
        }
    }
    ...
    protected function authenticate() {
        ...
        if (curl_error($ch)) {
            $error = curl_error($ch);
            curl_close($ch);
            console_log("âŒ cURL error during authentication", $error);
        ...
        if ($httpCode !== 200) {
            console_log("âŒ Authentication failed", "HTTP $httpCode");
        ...
        if (empty($this->token)) {
            console_log("âŒ Empty token received");
    ...
    private function makeRequest($endpoint) {
        ...
        if (curl_error($ch)) {
            $error = curl_error($ch);
            curl_close($ch);
            console_log("âŒ cURL error in API request", $error);
        ...
        if ($httpCode !== 200) {
            console_log("âŒ API request failed", "HTTP $httpCode");
            throw new Exception("API request failed with HTTP code: $httpCode. Response: $response");
        }

        $data = json_decode($response, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            console_log("âŒ JSON decode error", json_last_error_msg());
    ...
    private function loadStaticProducts() {
        ...
        if (!$staticProductsFile) {
            console_log("âŒ Static products file not found in any location");
            return [];
        }
        ...
        if ($content === false) {
            console_log("âŒ Failed to read static products file");
            return [];
        }
        ...
        if (preg_match('/export\s+const\s+staticProducts\s*=\s*(\[.*?\]);/s', $content, $matches)) {
            ...
            console_log("âŒ Could not find export pattern in file");
            return [];
        }
    }
    ...
    private function transformCockpit3dProduct($rawProduct, $catalog = []) {
        ...
        if (!$isLightbaseProduct) {
            ...
            if (!$isKeychainOrOrnament && isset($rawProduct['options']) && is_array($rawProduct['options'])) {
    ...
    private function saveRawProductsFile($products) {
        ...
        if ($result === false) {
            console_log("âŒ Failed to write raw products file", $rawProductsFile);
            return false;
        }
    ...
    private function saveRawCatalogFile($catalog) {
        ...
        if ($result === false) {
            console_log("âŒ Failed to write raw catalog file", $rawCatalogFile);
            return false;
        }
    ...
    public function generateRawFiles($refresh = false) {
        ...
        try {
            ...
            console_log("âŒ Error generating raw files", $e->getMessage());
            return [
                'success' => false,
                'error' => $e->getMessage()
            ];
    ...
    public function generateProcessedProductsFile($refresh = false) {
        ...
        try {
            ...
            if (file_exists($rawProductsFile) && file_exists($rawCatalogFile)) {
                ...
                if (preg_match('/export\s+const\s+cockpit3dRawProducts\s*=\s*(\[.*?\]);/s', $rawProductsContent, $matches)) {
                    ...
                    if (preg_match('/export\s+const\s+cockpit3dRawCatalog\s*=\s*(\[.*?\]);/s', $rawCatalogContent, $matches)) {
                        ...
                        console_log("âŒ Could not parse raw catalog content");
                    }
                } else {
                    console_log("âŒ Could not parse raw products content");
                }
            } else {
            ...
            console_log("âŒ Error generating processed products file", $e->getMessage());
            return [
                'success' => false,
                'error' => $e->getMessage()
            ];
...
if (basename($_SERVER['PHP_SELF']) === basename(__FILE__)) {
    try {
        ...
        console_log("âŒ Request failed", $e->getMessage());
