...
try {
    ...
    // ALWAYS collect shipping address (needed for Cockpit3D fulfillment)
    $sessionParams['shipping_address_collection'] = [
        'allowed_countries' => ['US', 'CA'],
    ];
    
    // Add shipping RATES and TAX only for production
    if ($mode === 'production') {
        ...
        error_log('✅ Production mode: Shipping ADDRESS + RATES + TAX enabled');
    } else {
        // Test/Development mode: Collect address but NO shipping rates or tax
        // This allows Cockpit3D data collection without requiring test shipping rate setup
        error_log('⚠️  Test mode: Shipping ADDRESS collected (for Cockpit3D), but NO rates/tax');
        error_log('    Customer will enter address but won\'t be charged for shipping');
    }
